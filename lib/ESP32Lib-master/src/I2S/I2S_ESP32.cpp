...
bool I2S::initParallelOutputMode(const int *pinMap, const int *pinMapBit, const int pinCount, long sampleRate, const int bitCount, int wordSelect, int baseClock)
{
	...
	if(sampleRate > 0)
	{
		...
		// Note: Here I, Ahnaf, updated a thing, that is I commented the upper line and uncommented the second upper line for it was throwing error(s).
		// rtc_clk_apll_coeff_set(odir, sdm & 255, (sdm >> 8) & 255, sdm >> 16);
		// Note: I, Ahnaf, also commented the upper line for it is in no use in the latest esp core.
	}
